...
slice(df, 20) #row 20
slice(df, 1:20) #row 1 to 20 using the colon operator
...
df %>% 
  select(name, gender, skin_color, mass) %>% 
  drop_na(gender) %>% 
  rename(skin = skin_color) %>% 
  slice_sample(n = 3)
...
#c) report the length of the new dataframe using the ´nrow()´ function
